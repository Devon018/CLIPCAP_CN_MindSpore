...
def generate(model, clip_embeds, tokenizer, args):
    ...
    while True:
        out = model.language_model(inputs_embeds=inputs_embeds)
    ...
    for caption_id in caption_ids:
        ...
        # caption = tokenizer.decode(caption_id)
...
def main(args):
    ...
    # 初始化模型
    language_model = ClipCaptionModel(tokenizer, args.prefix_len, args.clip_size, args.constant_len)
    # 加载权重

    params_dict = mindspore.load_checkpoint(args.checkpoint_file_path)
    params_not_load,_ = mindspore.load_param_into_net(language_model, params_dict)
    logger.info(f'Successfully loaded parameters from {args.checkpoint_file_path}')
    language_model.set_train(False)


    # 加载数据集
    dataset = ImageDataset(args.image_path)
    dataloader = GeneratorDataset(dataset,column_names=["name", "embedding"])
    ...
    for batch_idx, data in enumerate(tqdm(dataloader)):
        name,clip_embeds = data
        clip_embeds = clip_embeds.unsqueeze(1).tile((1, args.num_generate, 1)).view(-1, clip_embeds.shape[-1])
        captions = generate(language_model, clip_embeds, tokenizer, args)

        # 每num_generate个caption对应一张图片
        captions = ['\t'.join(captions[i: i+args.num_generate]) for i in range(0, clip_embeds.shape[0], args.num_generate)]
        captions_generate += captions
        image_name_list += name

    save_path = join(args.output_path, 'caption_generate.txt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_path', default='datasets/test/test_data.pkl')
    ...
    parser.add_argument('--checkpoint_file_path', default='outputs/checkpoint_final.ckpt')
